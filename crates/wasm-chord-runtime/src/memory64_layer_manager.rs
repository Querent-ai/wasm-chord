...
use crate::transformer::{TransformerLayer, TransformerConfig};
use wasm_chord_core::error::{Error, Result};
use std::sync::Arc;
use std::collections::HashMap;

/// Memory64-aware layer manager
pub struct Memory64LayerManager {
    ...
    /// Cache for loaded layers (layer_id -> layer_data)
    layer_cache: HashMap<u32, TransformerLayer>,
...
impl Memory64LayerManager {
    ...
    pub fn load_layer(&mut self, layer_id: u32) -> Result<&TransformerLayer> {
        ...
        if self.layer_cache.contains_key(&layer_id) {
            return Ok(self.layer_cache.get(&layer_id).unwrap());
        }
        ...
        // Store in cache
        self.layer_cache.insert(layer_id, layer);
        
        // Return reference to cached layer
        Ok(self.layer_cache.get(&layer_id).unwrap())
    }
    ...
    fn parse_layer_data(&self, layer_id: u32, data: &[f32]) -> Result<TransformerLayer> {
        ...
        if data.len() < expected_size {
            return Err(Error::ParseError(format!(
                ...
                layer_id, expected_size, data.len()
            )));
        }
    ...
    pub fn cache_stats(&self) -> CacheStats {
        CacheStats {
            cached_layers: self.layer_cache.len(),
