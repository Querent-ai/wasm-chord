...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Predicted token: '{}' (id: {}, logit: {:.6})", predicted_text, max_token, max_logit);
...
impl RMSNormDebugModel {
    ...
    fn debug_rmsnorm_forward(&self, token_ids: &[u32], position: usize) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        ...
        for layer_idx in 0..self.model.config.num_layers {
            ...
            hidden_states = self.debug_rms_norm(&hidden_states, &self.model.layers[layer_idx].attention_norm, 
            ...
            println!("\n   📐 Pre-FFN RMSNorm");
            let norm_input = hidden_states.clone();
            hidden_states = self.debug_rms_norm(&hidden_states, &self.model.layers[layer_idx].ffn_norm, 
        ...
        hidden_states = self.debug_rms_norm(&hidden_states, &self.model.output_norm, "Final Output".to_string())?;
    ...
    fn debug_rms_norm(&self, input: &[f32], weight: &[f32], name: String) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        let hidden_size = self.model.config.hidden_size;
        ...
        let weight_std = {
            let variance = weight.iter().map(|&x| (x - weight_mean).powi(2)).sum::<f32>() / weight.len() as f32;
            variance.sqrt()
        };
        ...
        println!("       Mean: {:.6}, Std: {:.6}, Min: {:.6}, Max: {:.6}, Max Abs: {:.6}", 
        ...
        let amplification_factor = if input_max_abs > 0.0 { output_max_abs / input_max_abs } else { 0.0 };
    ...
    fn print_tensor_stats(&self, name: &str, tensor: &[f32]) {
        ...
        println!("       {}: mean={:.6}, std={:.6}, min={:.6}, max={:.6}, max_abs={:.6}", 
                 name, mean, std, min, max, max_abs);
        
        // Show first few values
        let preview_len = 3.min(tensor.len());
        let preview: Vec<String> = tensor[..preview_len].iter().map(|&x| format!("{:.6}", x)).collect();
