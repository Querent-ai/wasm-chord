...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, &token_id) in tokens.iter().enumerate() {
        ...
        hidden_states[start_idx..end_idx].copy_from_slice(
            &model.token_embeddings[(token_id as usize * hidden_size)..((token_id as usize + 1) * hidden_size)]
        );
    }
    ...
    println!("   Hidden states stats: sum={:.6}, mean={:.6}, min={:.6}, max={:.6}", sum, mean, min, max);
    ...
    let max = hidden_states.iter().copied().fold(f32::NEG_INFINITY, f32::max);
    let min = hidden_states.iter().copied().fold(f32::INFINITY, f32::min);
    println!("   After RMSNorm stats: sum={:.6}, mean={:.6}, min={:.6}, max={:.6}", sum, mean, min, max);
    ...
    println!("   LM head stats: sum={:.6}, mean={:.6}, min={:.6}, max={:.6}", lm_sum, lm_mean, lm_min, lm_max);
    ...
    for (token_id, token_name) in test_tokens.iter().zip(token_names.iter()) {
        if *token_id < logits.len() {
            println!("   Model logit for '{}' (token {}): {:.6}", token_name, token_id, logits[*token_id as usize]);
        }
    }
    ...
    println!("\n🏆 Final Result:");
    println!("===============");
    println!("   Top token: {} (id: {}, logit: {:.6})", token_text.unwrap_or("<unknown>"), max_token, max_logit);
