...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Find top 10 tokens
    let mut token_logits: Vec<(u32, f32)> = logits.iter().enumerate()
        .map(|(i, &logit)| (i as u32, logit))
        .collect();
    ...
    for (i, (token_id, logit)) in token_logits.iter().take(10).enumerate() {
        ...
        println!("{:2}. {:12} (ID: {:5}) → logit: {:.6}", 
                 i + 1, token_text, token_id, logit);
    }
    ...
    println!("\n🎯 Prediction Analysis:");
    println!("=======================");
    println!("Our prediction: '{}' (ID: {}) → logit: {:.6}", 
             predicted_token_text, predicted_token_id, token_logits[0].1);
    println!("Expected (Ollama): '{}' (ID: {}) → logit: {:.6}", 
