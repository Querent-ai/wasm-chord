...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match tokenizer.encode(prompt, false) {
            Ok(tokens) => {
                ...
                let decoded: Vec<String> = tokens.iter()
                    .map(|&token_id| tokenizer.id_to_token(token_id as u32).unwrap_or("<unknown>").to_string())
                    .collect();
                println!("              → {:?}", decoded);
            }
            Err(e) => {
    ...
    let common_tokens = vec![
        "Hello", "hello", "The", "the", "A", "a", "I", "i", "You", "you",
        "Yes", "yes", "No", "no", "Good", "good", "Bad", "bad",
        "番", "Yes", "No", "Hello", "World", "world"
    ];
    ...
    for token in common_tokens {
        if let Ok(tokens) = tokenizer.encode(token, false) {
            if let Some(&token_id) = tokens.first() {
                let decoded = tokenizer.id_to_token(token_id as u32);
