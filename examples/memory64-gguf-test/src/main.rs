...
use wasm_chord_runtime::{
    memory64_gguf::Memory64GGUFLoader,
    TransformerConfig,
};
...
fn test_model_loading(model_path: &str) -> Result<()> {
    ...
    let file = File::open(model_path)
        .map_err(|e| wasm_chord_core::error::Error::ParseError(format!("Failed to open {}: {}", model_path, e)))?;
    ...
    println!("   ✅ GGUF parsed: {} tensors, architecture: {}", 
    ...
    let file = File::open(model_path)
        .map_err(|e| wasm_chord_core::error::Error::ParseError(format!("Failed to reopen {}: {}", model_path, e)))?;
    let reader = BufReader::new(file);
    let mut parser = GGUFParser::new(reader);
    ...
    match loader.load_model(&mut parser) {
        Ok(mut model) => {
            ...
            println!("   📊 Cache stats: {} layers cached (max: {})", 
