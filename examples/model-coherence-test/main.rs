...
fn assess_coherence(result: &str, _prompt: &str) -> bool {
    // Basic coherence checks
    let words: Vec<&str> = result.split_whitespace().collect();
    ...
    let english_like_words = words
        .iter()
        .filter(|&&word| word.chars().all(|c| c.is_alphabetic() || c == '\'') && word.len() >= 2)
        .count();
