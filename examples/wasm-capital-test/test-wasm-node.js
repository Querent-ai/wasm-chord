...
// Configuration
const MODEL_PATH = path.join(__dirname, 'models', 'tinyllama-1.1b.Q4_K_M.gguf');
const WASM_PKG_PATH = path.join(__dirname, 'runtime-pkg-node');
...
async function main() {
    ...
    const wasmJsPath = path.join(WASM_PKG_PATH, 'wasm_chord_runtime.js');
    const wasmBinPath = path.join(WASM_PKG_PATH, 'wasm_chord_runtime_bg.wasm');
    const wasmJsExists = fs.existsSync(wasmJsPath);
    const wasmBinExists = fs.existsSync(wasmBinPath);
