...
impl DebugModel {
    ...
    fn debug_forward(&self, token_ids: &[u32], position: usize) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    ...
    fn debug_rms_norm(&self, input: &[f32], weight: &[f32]) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
        let hidden_size = self.model.config.hidden_size;
    ...
    fn debug_attention(&self, input: &[f32], _layer_idx: usize, _position: usize) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    ...
    fn debug_ffn(&self, input: &[f32], _layer_idx: usize) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    ...
    fn print_tensor_diff(&self, name: &str, before: &[f32], after: &[f32]) {
        if before.len() != after.len() {
            println!("     {}: Length mismatch! before={}, after={}", name, before.len(), after.len());
            return;
        }
