...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test specific weight matrices
    test_weight_matrix("token_embeddings", &meta, &config)?;
...
fn test_weight_matrix(
    tensor_name: &str, 
    meta: &wasm_chord_core::GGUFMetadata,
    config: &TransformerConfig
) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🧪 Testing tensor: {}", tensor_name);
    
    // Find tensor in metadata
    let tensor_desc = meta.tensors.iter()
    ...
    let gguf_shape = &tensor_desc.shape;
    if gguf_shape.len() == 2 {
